    ...
    ISablierLidoAdapter 
{
        ...
        Comptrollerable(initialComptroller)
    {
        ...
        if (initialSlippageTolerance.gt_1(MAX_SLIPPAGE_TOLERANCE)) {
            revert Errors_1.SablierLidoAdapter_SlippageToleranceTooHigh(initialSlippageTolerance, MAX_SLIPPAGE_TOLERANCE);
        }

        
        if (initialYieldFee.gt_1(MAX_FEE)) {
            revert Errors_1.SablierLidoAdapter_YieldFeeTooHigh(initialYieldFee, MAX_FEE);
        }
    ...
    modifier onlySablierBob() {
        if (msg.sender != SABLIER_BOB) {
            revert Errors_1.SablierLidoAdapter_OnlySablierBob(msg.sender, SABLIER_BOB);
        }
        _;
    }
        ...
        returns (uint128 transferAmount, uint128 feeAmountDeductedFromYield)
    {
        ...
        if (userWethShare > shareBalance) {
            ...
            feeAmountDeductedFromYield = ud(yieldAmount).mul_1(_vaultYieldFee[vaultId]).intoUint128_5();
    ...
    function requestLidoWithdrawal(uint256 vaultId) external override onlyComptroller {
        ...
        if (ISablierBobState(SABLIER_BOB).statusOf(vaultId) == Bob.Status.ACTIVE) {
            revert Errors_1.SablierLidoAdapter_VaultActive(vaultId);
        }

        
        if (!ISablierBobState(SABLIER_BOB).isStakedInAdapter(vaultId)) {
            revert Errors_1.SablierLidoAdapter_VaultAlreadyUnstaked(vaultId);
        }

        
        if (_lidoWithdrawalRequestIds[vaultId].length > 0) {
            revert Errors_1.SablierLidoAdapter_LidoWithdrawalAlreadyRequested(vaultId);
        }
        ...
        if (totalWstETH == 0) {
            revert Errors_1.SablierLidoAdapter_NoWstETHToWithdraw(vaultId);
        }
        ...
        if (stETHAmount < minAmountPerRequest) {
            revert Errors_1.SablierLidoAdapter_WithdrawalAmountBelowMinimum(vaultId, stETHAmount, minAmountPerRequest);
        }
    ...
    function setSlippageTolerance(UD60x18 newTolerance) external override onlyComptroller {
        ...
        if (newTolerance.gt_1(MAX_SLIPPAGE_TOLERANCE)) {
            revert Errors_1.SablierLidoAdapter_SlippageToleranceTooHigh(newTolerance, MAX_SLIPPAGE_TOLERANCE);
        }
    ...
    function setYieldFee(UD60x18 newFee) external override onlyComptroller {
        ...
        if (newFee.gt_1(MAX_FEE)) {
            revert Errors_1.SablierLidoAdapter_YieldFeeTooHigh(newFee, MAX_FEE);
        }
        ...
        onlySablierBob
    {
        ...
        if (userShareBalanceBeforeTransfer == 0) {
            revert Errors_1.SablierLidoAdapter_UserBalanceZero(vaultId, from);
        }
        ...
        if (wstETHToTransfer == 0) {
            revert Errors_1.SablierLidoAdapter_WstETHTransferAmountZero(vaultId, from, to);
        }
    ...
    function _swapWstETHToWeth(uint128 wstETHAmount) private returns (uint128 wethReceived) {
        ...
        if (oraclePrice == 0) {
            revert Errors_1.SablierLidoAdapter_OraclePriceZero();
        }
        ...
        uint256 minEthOut = ud(fairEthOut).mul_1(UNIT_6.sub_1(slippageTolerance)).unwrap_5();
        ...
        if (ethReceived < minEthOut) {
            revert Errors_1.SablierLidoAdapter_SlippageExceeded(minEthOut, ethReceived);
        }
