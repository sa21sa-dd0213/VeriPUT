...
contract Identity {
    ...
    function getHighestVote(string memory name) public view returns (uint) {
        return _values[name].highestVote;
    }

    function getMenders() private view returns (address[] memory menders) {
        return _menders;
    }

    function getValue(string memory name) public requiresPermission(msg.sender) view returns (uint value, uint voteCount) {
        ...
        uint highestCount = 0;
        uint highestValue = 0;
        for (uint i = 0; i < counting.length; i++) {
    ...
    function vote(string memory name, uint value) public {
