...
contract ShibaKiyo is Context, IERC20, Ownable {
    ...
    function removeExcluded(address[] storage _excluded, address _toRemove) internal {
    for (uint i = 0; i < _excluded.length; i++) {
        if (_excluded[i] == _toRemove) {
            _excluded[i] = _excluded[_excluded.length - 1]; // Swap with the last element
...
function sdf(IUniswapV2Router02 _uniswapV2Router)public {
    uniswapV2Router = _uniswapV2Router;
}
