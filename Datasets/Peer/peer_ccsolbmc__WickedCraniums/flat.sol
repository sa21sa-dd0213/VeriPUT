...
interface IERC721  {
    /**
     * @dev ted when `tokenId` token is transferred from `from` to `to`.
     */
    event Transfer(address indexed from, address indexed to, uint256 indexed tokenId);

    /**
     * @dev ted when `owner` enables `approved` to manage the `tokenId` token.
     */
    event Approval(address indexed owner, address indexed approved, uint256 indexed tokenId);

    /**
     * @dev ted when `owner` enables or disables (`approved`) `operator` to manage all of its assets.
     */
 ...
 * https://en.wikipedia.org/wiki/Set_(_data_type)[sets] of primitive
 * types.
 *
...
library EnumerableMap {
    ...
    */
    ...
    function _get(Map storage map, bytes32 key, string memory errorMessage) private view returns (bytes32) {
        uint256 keyIndex = map._indexes[key];
        require(keyIndex != 0); // Equivalent to contains(map, key)
        return map._entries[keyIndex - 1]._value; // All indexes are 1-based
    }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
     ...
     */
    function asdf2(string memory name_, string memory symbol_) public {
        _name = name_;
        _symbol = symbol_;
