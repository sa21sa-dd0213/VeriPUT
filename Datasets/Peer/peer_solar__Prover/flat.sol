...
library Sets {
    ...
    function insert(addressSet storage self, address other) internal {
    ...
    function remove(addressSet storage self, address other) internal {
        if (contains(self, other)) {
    ...
    function contains(addressSet storage self, address other) internal view returns (bool) {
    ...
    function insert(uintSet storage self, uint other) internal {
        if (!contains(self, other)) {
            assert(length(self) < 2**256-1);
            self.members.push(other);
            self.memberIndices[other] = length(self);
    ...
    function remove(uintSet storage self, uint other) internal {
        if (contains(self, other)) {
            uint replaceIndex = self.memberIndices[other];
    ...
    function contains(uintSet storage self, uint other) internal view returns (bool) {
        return self.memberIndices[other] > 0;
    }

    function length(uintSet storage self) internal view returns (uint) {
        return self.members.length;
    }
    ...
    function insert(uint8Set storage self, uint8 other) internal {
        if (!contains(self, other)) {
            assert(length(self) < 2**256-1);
            self.members.push(other);
            self.memberIndices[other] = length(self);
    ...
    function remove(uint8Set storage self, uint8 other) internal {
        if (contains(self, other)) {
            uint replaceIndex = self.memberIndices[other];
    ...
    function contains(uint8Set storage self, uint8 other) internal view returns (bool) {
        return self.memberIndices[other] > 0;
    }

    function length(uint8Set storage self) internal view returns (uint) {
        return self.members.length;
    }
    ...
    function insert(intSet storage self, int other) internal {
        if (!contains(self, other)) {
            assert(length(self) < 2**256-1);
            self.members.push(other);
            self.memberIndices[other] = length(self);
    ...
    function remove(intSet storage self, int other) internal {
        if (contains(self, other)) {
            uint replaceIndex = self.memberIndices[other];
    ...
    function contains(intSet storage self, int other) internal view returns (bool) {
        return self.memberIndices[other] > 0;
    }

    function length(intSet storage self) internal view returns (uint) {
        return self.members.length;
    }
    ...
    function insert(int8Set storage self, int8 other) internal {
        if (!contains(self, other)) {
            assert(length(self) < 2**256-1);
            self.members.push(other);
            self.memberIndices[other] = length(self);
    ...
    function remove(int8Set storage self, int8 other) internal {
        if (contains(self, other)) {
            uint replaceIndex = self.memberIndices[other];
    ...
    function contains(int8Set storage self, int8 other) internal view returns (bool) {
        return self.memberIndices[other] > 0;
    }

    function length(int8Set storage self) internal view returns (uint) {
        return self.members.length;
    }
    ...
    struct byteSet {
        bytes1[] members;
        mapping(bytes1 => uint) memberIndices;
    }

    function insert(byteSet storage self, bytes1 other) internal {
        if (!contains(self, other)) {
            assert(length(self) < 2**256-1);
            self.members.push(other);
            self.memberIndices[other] = length(self);
    ...
    function remove(byteSet storage self, bytes1 other) internal {
        if (contains(self, other)) {
            uint replaceIndex = self.memberIndices[other];
            bytes1 lastMember = self.members[length(self)-1];
            // overwrite other with the last member and remove last member
            self.members[replaceIndex-1] = lastMember;
            self.members.pop();
            // reflect this change in the indices
    ...
    function contains(byteSet storage self, bytes1 other) internal view returns (bool) {
        return self.memberIndices[other] > 0;
    }

    function length(byteSet storage self) internal view returns (uint) {
        return self.members.length;
    }
    ...
    function insert(bytes32Set storage self, bytes32 other) internal {
        if (!contains(self, other)) {
            assert(length(self) < 2**256-1);
            self.members.push(other);
            self.memberIndices[other] = length(self);
    ...
    function remove(bytes32Set storage self, bytes32 other) internal {
        if (contains(self, other)) {
            uint replaceIndex = self.memberIndices[other];
    ...
    function contains(bytes32Set storage self, bytes32 other) internal view returns (bool) {
        return self.memberIndices[other] > 0;
    }

    function length(bytes32Set storage self) internal view returns (uint) {
        return self.members.length;
    }
}
...
contract Prover {
    ...
    function registeredUsers()
        external
        view
        returns (address[] memory unique_addresses) {
        return users.members;
    }
    function userEntries(address target)
        external
        view
        returns (bytes32[] memory entries) {
    ...
    function addEntry(bytes32 dataHash)
        ...
        accounts[msg.sender].values[dataHash] = Entry(block.timestamp, msg.value);
    }
    ...
    function deleteEntry(bytes32 dataHash)
        ...
        // send the rebate
        if (rebate > 0) payable(msg.sender).transfer(rebate);
    }
