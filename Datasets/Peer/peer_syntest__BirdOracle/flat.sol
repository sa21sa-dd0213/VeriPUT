...
contract BirdOracle {
   ...
   */
    ...
    public
    {
        // [manual 0.8 port] BirdRequest holds two mappings, so it can no longer be
        // constructed as a value and pushed. Reserve the slot, then fill the
        // non-mapping fields in place -- the mappings start empty either way, which
        // is exactly what the value-constructed version produced.
        onChainRequests.push();
        uint length = onChainRequests.length;
