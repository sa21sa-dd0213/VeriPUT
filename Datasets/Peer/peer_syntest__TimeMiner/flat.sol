...
contract TimeMiner {
    ...
    constructor()  {
        ...
        preSaleInfo.admin = payable(msg.sender);
    ...
    function totalSupply() public view returns (uint256) {
        uint256 countOfCoinsToAdd = ((block.timestamp - info.coinCreationTime) / 1 hours);
    ...
    function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
        ...
        if(info.stableCoinSystem){
            ...
            // If time left
            if(info.coinWorkingTime + 1 hours < block.timestamp)
            {
                uint256 countOfCoinsToAdd = ((block.timestamp - info.coinCreationTime) / 1 hours);
                info.coinWorkingTime = block.timestamp;
