...
interface IRoundFactory {
    function create(bytes calldata encodedParameters, address ownedBy) external returns (address);
    function updateRoundImplementation(address payable newRoundImplementation) external;
    function updateAlloSettings(address newAlloSettings) external;
}
interface IRoundImplementation {
    function initialize(bytes calldata encodedParameters, address alloSettings) external;
}
struct MetaPtr {
    uint256 protocol;
    string pointer;
