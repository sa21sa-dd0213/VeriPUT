// SPDX-License-Identifier: MIT
pragma solidity >=0.7.0;

import "forge-std/Test.sol";
import "../src/Target.sol";

contract CCSolBMCDerived is Test {
    WrappedToken target;
    address caller = address(0xCA11E4);

    receive() external payable {}

    function setUp() public {



        target = new WrappedToken();
    }

    function test_ce_0_transfer() public {


        target.transfer(address(uint160(0)), 0);
    }
    function test_ce_1_transfer() public {


        target.transfer(address(uint160(1)), 0);
    }
    function test_ce_2_approve() public {


        target.approve(address(uint160(0)), 0);
    }
    function test_ce_3_approve() public {


        target.approve(address(uint160(1)), 0);
    }
    function test_ce_4_transferFrom() public {


        target.transferFrom(address(uint160(1)), address(uint160(1)), 1);
    }
    function test_ce_5_decreaseAllowance() public {


        target.decreaseAllowance(address(uint160(0)), 0);
    }
    function test_ce_6_decreaseAllowance() public {


        target.decreaseAllowance(address(uint160(0)), 39);
    }
    function test_ce_7_wrap() public {


        target.wrap(0);
    }
    function test_ce_8_unwrap() public {


        target.unwrap(0);
    }
    function test_ce_9_unwrap() public {


        target.unwrap(1);
    }
    function test_ce_10_revokeRole() public {


        target.revokeRole(bytes32(uint256(38)), address(uint160(0)));
    }
    function test_ce_11_abca2() public {


        target.abca2();
    }
    function test_ce_12_renounceRole() public {


        target.renounceRole(bytes32(uint256(7719)), address(uint160(38)));
    }
    function test_ce_13_renounceRole() public {


        target.renounceRole(bytes32(uint256(0)), address(uint160(0)));
    }
    function test_ce_14_setPaused() public {


        target.setPaused(false);
    }
    function test_ce_15_setPaused() public {


        target.setPaused(true);
    }
    function test_ce_16_transferOwnership() public {


        target.transferOwnership(address(uint160(0)));
    }
    function test_ce_17_setHeldAmtReq() public {


        target.setHeldAmtReq(0);
    }
    function test_ce_18_transferBackingTokenFrom() public {


        target.transferBackingTokenFrom(address(uint160(0)), 0);
    }
    function test_ce_19_transferBackingTokenTo() public {


        target.transferBackingTokenTo(address(uint160(0)), 0);
    }
    function test_ce_20_mintTo() public {


        target.mintTo(address(uint160(0)), 0);
    }
    function test_ce_21_burnFrom() public {


        target.burnFrom(address(uint160(0)), 0);
    }
}
