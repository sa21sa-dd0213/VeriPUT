...
contract KizunaInuCovTest_KizunaInu_startTrading_put6p1 is Test {
  KizunaInu c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint8 s_cooldownEnabled, uint8 s_swapEnabled, uint8 s_tradingOpen) internal {
    ...
    assertLe(((uint256(vm.load(address(c0), bytes32(uint256(15)))) >> 176) & 255), uint256(1), "the entry state is OUTSIDE the certified region: state.swapEnabled was assumed in [0, 1] when the path was certified, so a value outside it means the assumption was vacuous and the rungs below were proved about no execution this test can reach");
    {
      uint256 _w = uint256(vm.load(address(c0), bytes32(uint256(15))));
    ...
    bool _put_ok = true;
    try c0.startTrading() {} catch { _put_ok = false; }
  ...
  function test_put_KizunaInu_startTrading_path6p1(address p_msg_sender, uint8 s_cooldownEnabled, uint8 s_swapEnabled, uint8 s_tradingOpen) public {
    _veriput_parameterized(p_msg_sender, s_cooldownEnabled, s_swapEnabled, s_tradingOpen);
    
    { KizunaInuCovTest_KizunaInu_startTrading_concrete6p1__basis_root__W _veriput_w = new KizunaInuCovTest_KizunaInu_startTrading_concrete6p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_1(); }
}
}
...
contract KizunaInuCovTest_KizunaInu_startTrading_concrete6p1__basis_root__W is Test {
  KizunaInu c0;
  function setUp() public {
    c0 = new KizunaInu();
  }
