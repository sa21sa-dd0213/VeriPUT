...
contract RubixiCovTest_Rubixi_fallback_put6p1_p1_part_part0_w_w is Test {
  ...
  //   environment ESTABLISHED: msg.sender in [0, 4294967293]  \ {0} is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it, and the 1 punched value(s) are excluded by vm.assume; 0 is excluded because Foundry cannot prank address(0), and the remaining executable subregion is a subset of the certified region (no prank was present, so one was inserted)
  ...
  uint256 _pre_balance; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _pre_collectedFees; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _pre_feePercent; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _pre_pyramidMultiplier; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _pre_payoutOrder; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _post_balance; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _post_collectedFees; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _post_feePercent; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _post_pyramidMultiplier; // VERIPUT_HOISTED_SNAPSHOT
  uint256 _post_payoutOrder; // VERIPUT_HOISTED_SNAPSHOT
