...
contract RubixiCovTest_Rubixi_collectFeesInEther_put2p1 is Test {
  Rubixi c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 _amt) internal {
    p_msg_sender = address(uint160(bound(uint256(uint160(p_msg_sender)), 1, 1461501637330902918203684832716283019655932542975)));
    p_msg_value = bound(p_msg_value, 1, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
    _amt = bound(_amt, 0, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("collectFeesInEther(uint256)", _amt));
  ...
  function test_put_Rubixi_collectFeesInEther_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 _amt) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, _amt);
  }
}
