...
contract FixedSupplyTokenCovTest_FixedSupplyToken_approve_put2p1 is Test {
  FixedSupplyToken c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address _spender, uint256 _amount) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("approve(address,uint256)", _spender, _amount));
  ...
  function test_put_FixedSupplyToken_approve_path2p1(address p_msg_sender, uint256 p_msg_value, address _spender, uint256 _amount) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, _spender, _amount);
  }
}
