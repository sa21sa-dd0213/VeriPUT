...
contract wLitiSaleCovTest_wLitiSale_getETHWallet_put3p1 is Test {
  ...
  // claim: sol:@C@wLitiSale@F@getETHWallet#553:path:3
  // witness-fingerprint-sha256: 5db4378c9f974c45dfc12720c68f88f8c741a45500e60451115c0374963de049
  // claim: sol:@C@wLitiSale@F@getETHWallet#553:path:2
  // witness-fingerprint-sha256: 9bf26a15184fdc98578e2ec0b1f03f0f303d530f2d43067ed8e62833fdedfaee

  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@wLitiSale@F@getETHWallet#553:path:3   depth=1
  ...
  //   state._ETHWallet in [0, 1461501637330902918203684832716283019655932542975]
  //   PIN block.basefee == 0
  ...
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post <= pre (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post == (pre + msg.value) (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [msg.value, msg.value] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, pre] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (pre + pre)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (pre * pre)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (pre + 1461501637330902918203684832716283019655932542975)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (pre * 1461501637330902918203684832716283019655932542975)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (msg.value - pre)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (msg.value + 1461501637330902918203684832716283019655932542975)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (1461501637330902918203684832716283019655932542975 - pre)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (pre / 1461501637330902918203684832716283019655932542975)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (msg.value / 1461501637330902918203684832716283019655932542975)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _ETHWallet: post in [0, (1461501637330902918203684832716283019655932542975 / 1461501637330902918203684832716283019655932542975)] (entailed by a stronger rung that also HOLDS on _ETHWallet, so asserting it detects nothing the stronger one misses)
  //   entry-state bound DROPPED: state._ETHWallet in [0, 1461501637330902918203684832716283019655932542975] (width > 1, DROPPED: the entry state is not havoc'd, so this bound constrained nothing in the query -- the rungs were proved about the constructor's own value. Establishing a fuzz-chosen value here would test entry states the proof never covered, which is how this PUT came back RED on the unmodified contract. Both endpoints are the type's own limits, so there is not even an in-region check to make: the bound says nothing at all)
