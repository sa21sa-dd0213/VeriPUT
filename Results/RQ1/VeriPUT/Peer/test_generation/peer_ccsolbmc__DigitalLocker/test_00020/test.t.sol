...
contract DigitalLockerCovTest_DigitalLocker_RequestLockerAccess_put7p1_p1_part_part0_w_r is Test {
  ...
  //   environment ESTABLISHED: msg.sender in [2147483647, 4294967294] is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it (no prank was present, so one was inserted)
