...
contract DigitalLockerCovTest_DigitalLocker_RequestLockerAccess_put7p1_p1_part_part0_w_w is Test {
  ...
  //   environment ESTABLISHED: msg.sender in [1, 2147483646] is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it (no prank was present, so one was inserted)
