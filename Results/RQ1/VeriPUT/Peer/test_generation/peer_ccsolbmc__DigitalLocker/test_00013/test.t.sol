...
contract DigitalLockerCovTest_DigitalLocker_RequestLockerAccess_put2p1 is Test {
  DigitalLocker c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, string memory intendedPurpose) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("RequestLockerAccess(string)", intendedPurpose));
  ...
  function test_put_DigitalLocker_RequestLockerAccess_path2p1(address p_msg_sender, uint256 p_msg_value, string memory intendedPurpose) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, intendedPurpose);
  }
}
