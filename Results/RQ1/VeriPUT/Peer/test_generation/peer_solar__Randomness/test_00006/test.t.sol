...
contract RandomnessCovTest_Randomness_setSealedSeed_put2p1 is Test {
  Randomness c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 _sealedSeed) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("setSealedSeed(bytes32)", bytes32(_sealedSeed)));
  ...
  function test_put_Randomness_setSealedSeed_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 _sealedSeed) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, _sealedSeed);
  }
}
