...
contract RandomnessCovTest_0_Randomness_setSealedSeed_put15p1 is Test {
  ...
  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@Randomness@F@setSealedSeed#59:path:15   depth=3
  ...
  //   rung IMPLIED (not lost, not asserted twice): trustedParty: post >= pre (entailed by a stronger rung that also HOLDS on trustedParty, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): trustedParty: post <= pre (entailed by a stronger rung that also HOLDS on trustedParty, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): trustedParty: post == (pre + msg.value) (entailed by a stronger rung that also HOLDS on trustedParty, so asserting it detects nothing the stronger one misses)
  //   path guard ESTABLISHED: `!(msg.sender == trustedParty)` -> `vm.assume(uint256(uint160(p_msg_sender)) == _pre_trustedParty);`
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_block_number, uint256 _sealedSeed, uint8 s_betsClosed) internal {
    ...
    //   state.trustedParty := msg.sender; state.betsClosed := s_betsClosed in [0, 1] (SET and FUZZED: freed at the certification entry); state.seedSet := 0
    {
