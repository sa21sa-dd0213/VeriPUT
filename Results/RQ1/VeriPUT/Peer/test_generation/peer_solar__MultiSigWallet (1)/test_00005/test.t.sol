...
contract MultiSigWalletCovTest_MultiSigWallet_send_put1p1 is Test {
  ...
  function test_put_MultiSigWallet_send_path1p1(address p_msg_sender) public {
    ...
    { MultiSigWalletCovTest_MultiSigWallet_send_concrete1p1__basis_root__W _veriput_w = new MultiSigWalletCovTest_MultiSigWallet_send_concrete1p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract MultiSigWalletCovTest_MultiSigWallet_send_concrete1p1__basis_root__W is Test {
  MultiSigWallet c0;
  function setUp() public {
    vm.startPrank(address(uint160(1)), address(uint160(1)));
    c0 = new MultiSigWallet();
  ...
  function _w_test_cov_0() public {
    ...
    assertEq(uint256(vm.load(address(c0), bytes32(uint256(2)))), uint256(0), "entry pin state._transactionIdx did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    ...
    assertEq(_veriput_fixed_state_transactionIdx_2, uint256(0), "fixed witness state");
  }
}
