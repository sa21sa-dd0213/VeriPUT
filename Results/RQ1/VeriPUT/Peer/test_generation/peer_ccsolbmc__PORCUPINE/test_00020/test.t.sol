...
contract PORCUPINECovTest_PORCUPINE_transferOwnership_put14p1 is Test {
  PORCUPINE c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address newOwner) internal {
    ...
    assertEq((uint256(vm.load(address(c0), bytes32(uint256(0)))) & 1461501637330902918203684832716283019655932542975), uint256(uint256(uint160(p_msg_sender))), "entry pin state._owner$392 did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    ...
    vm.assume(_pre_owner_392 == uint256(uint160(p_msg_sender)));
  ...
  function test_put_PORCUPINE_transferOwnership_path14p1(address p_msg_sender, address newOwner) public {
    _veriput_parameterized(p_msg_sender, newOwner);
    
    { PORCUPINECovTest_PORCUPINE_transferOwnership_concrete14p1__basis_root__W _veriput_w = new PORCUPINECovTest_PORCUPINE_transferOwnership_concrete14p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_1(); }
}
}
...
contract PORCUPINECovTest_PORCUPINE_transferOwnership_concrete14p1__basis_root__W is Test {
  PORCUPINE c0;
  function setUp() public {
    c0 = new PORCUPINE();
  }
