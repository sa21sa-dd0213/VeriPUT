...
contract TimeMinerCovTest_TimeMiner_whitelist_put2p1 is Test {
  TimeMiner c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address _user, bool _status) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("whitelist(address,bool)", _user, _status));
  ...
  function test_put_TimeMiner_whitelist_path2p1(address p_msg_sender, uint256 p_msg_value, address _user, bool _status) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, _user, _status);
  }
}
