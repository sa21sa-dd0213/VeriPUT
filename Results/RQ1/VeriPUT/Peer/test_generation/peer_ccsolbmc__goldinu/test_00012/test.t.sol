...
contract goldinuCovTest_goldinu_setBots_put25p1 is Test {
  ...
  // claim: sol:@C@goldinu@F@setBots#1027:path:13, sol:@C@goldinu@F@setBots#1027:path:2, sol:@C@goldinu@F@setBots#1027:path:25, sol:@C@goldinu@F@setBots#1027:path:49, sol:@C@goldinu@F@setBots#1027:path:7
  // witness-fingerprint-sha256: c176b5492bd8fc3b17745aca290dbb7c222f76fb8249326c9efbe6dad33970c3

  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@goldinu@F@setBots#1027:path:25   depth=4
  ...
  //   level0 = True
  //   level0_perturb = True
  //   level0_points = {'13': 'bots_.length==1', '25': 'bots_.length==2', '49': 'bots_.length==3', '7': 'bots_.length==0'}
  //   max_holes = 3
  //   max_region_pieces = 4
  //   max_tx = 1
  //   probe_ladder = True
  ...
  // FUZZ COORDINATES: msg.sender, bots_
  ...
  //   rung DROPPED: state.bots_.length (no storage slot: solc's layout does not list it, so the R2 endpoint cannot be read before the call)
  //   rung DROPPED: state.bots_.length (no storage slot: solc's layout does not list it, so the R2 endpoint cannot be read before the call)
  //   rung DROPPED: state.bots_.length (no storage slot: solc's layout does not list it, so the R2 endpoint cannot be read before the call)
  //   path guard DROPPED: '!(i < bots_.length)' (`i` is assigned inside the unit before this source decision may execute, so it is not the calldata coordinate)
  //   path guard DROPPED: '!(i < bots_.length)' (`i` is assigned inside the unit before this source decision may execute, so it is not the calldata coordinate)
  //   path guard DROPPED: 'i < bots_.length' (`i` is assigned inside the unit before this source decision may execute, so it is not the calldata coordinate)
