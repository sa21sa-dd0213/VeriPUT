...
contract eMuppyCovTest_eMuppy_transfer_put14p1 is Test {
  eMuppy c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address recipient, uint256 amount) internal {
    p_msg_sender = address(uint160(bound(uint256(uint160(p_msg_sender)), 1, 1099511627776)));
  ...
  function test_put_eMuppy_transfer_path14p1(address p_msg_sender, address recipient, uint256 amount) public {
    _veriput_parameterized(p_msg_sender, recipient, amount);
    
    { eMuppyCovTest_eMuppy_transfer_concrete14p1__basis_root__W _veriput_w = new eMuppyCovTest_eMuppy_transfer_concrete14p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_2(); }
}
}
...
contract eMuppyCovTest_eMuppy_transfer_concrete14p1__basis_root__W is Test {
  eMuppy c0;
  function setUp() public {
    c0 = new eMuppy();
  }
