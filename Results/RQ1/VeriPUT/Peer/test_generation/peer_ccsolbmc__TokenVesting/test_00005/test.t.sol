...
contract TokenVestingCovTest_TokenVesting_move_into_tranche_put2p1 is Test {
  TokenVesting c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address user, uint8 tranche_id, uint256 amount) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("move_into_tranche(address,uint8,uint256)", user, tranche_id, amount));
  ...
  function test_put_TokenVesting_move_into_tranche_path2p1(address p_msg_sender, uint256 p_msg_value, address user, uint8 tranche_id, uint256 amount) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, user, tranche_id, amount);
  }
}
