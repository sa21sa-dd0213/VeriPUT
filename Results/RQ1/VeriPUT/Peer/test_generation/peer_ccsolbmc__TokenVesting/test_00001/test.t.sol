...
contract TokenVestingCovTest_TokenVesting_get_tranche_balance_put2p1 is Test {
  TokenVesting c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address user, uint8 tranche_id) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("get_tranche_balance(address,uint8)", user, tranche_id));
  ...
  function test_put_TokenVesting_get_tranche_balance_path2p1(address p_msg_sender, uint256 p_msg_value, address user, uint8 tranche_id) public {
