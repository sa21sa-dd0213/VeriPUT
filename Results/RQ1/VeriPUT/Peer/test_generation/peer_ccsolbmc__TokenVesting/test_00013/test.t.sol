...
contract TokenVestingCovTest_TokenVesting_revoke_issuer_put2p1 is Test {
  TokenVesting c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address issuer) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("revoke_issuer(address)", issuer));
  ...
  function test_put_TokenVesting_revoke_issuer_path2p1(address p_msg_sender, uint256 p_msg_value, address issuer) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, issuer);
  }
}
