...
contract AssetTransferCovTest_AssetTransfer_initialize_put2p1 is Test {
  AssetTransfer c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, string memory description, uint256 price) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("initialize(string,uint256)", description, price));
    ...
    assertLe(_post_AskingPrice, _pre_AskingPrice, "AskingPrice: post <= pre");
  ...
  function test_put_AssetTransfer_initialize_path2p1(address p_msg_sender, uint256 p_msg_value, string memory description, uint256 price) public {
