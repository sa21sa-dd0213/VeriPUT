...
contract AssetTransferCovTest_AssetTransfer_MakeOffer_put13p1_p1_part_part0_r is Test {
  AssetTransfer c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address inspector, address appraiser, uint256 offerPrice) internal {
    ...
    appraiser = address(uint160(bound(uint256(uint160(appraiser)), 0, 0)));
    ...
    if (p_msg_sender == address(uint160(0)) && inspector == address(uint160(0)) && appraiser == address(uint160(0)) && offerPrice == uint256(0)) {
    ...
    assertFalse(_put_ok, "path enc=13p1_part_part0_r exits through a REVERT: the call must fail on the unmodified contract");
  }


  
  function test_put_AssetTransfer_MakeOffer_path13p1_part_part0_r(address p_msg_sender, address inspector, address appraiser, uint256 offerPrice) public {
    _veriput_parameterized(p_msg_sender, inspector, appraiser, offerPrice);
    
    { AssetTransferCovTest_AssetTransfer_MakeOffer_concrete13p1__basis_part0_r__W _veriput_w = new AssetTransferCovTest_AssetTransfer_MakeOffer_concrete13p1__basis_part0_r__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_2(); }
}
}
...
contract AssetTransferCovTest_AssetTransfer_MakeOffer_concrete13p1__basis_part0_r__W is Test {
  AssetTransfer c0;
  function setUp() public {
    c0 = new AssetTransfer();
  }
  ...
  function _w_test_cov_2() public {
    ...
    try c0.MakeOffer(address(uint160(0)), address(uint160(0)), uint256(0)) {
