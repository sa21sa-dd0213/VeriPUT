...
contract AssetTransferCovTest_AssetTransfer_MarkInspected_put31p1 is Test {
  AssetTransfer c0;
  ...
  function _veriput_parameterized(address p_msg_sender) internal {
    ...
    assertEq((uint256(vm.load(address(c0), bytes32(uint256(5)))) & 1461501637330902918203684832716283019655932542975), uint256(uint256(uint160(p_msg_sender))), "entry pin state.InstanceInspector did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    ...
    vm.assume(_pre_InstanceInspector == uint256(uint160(p_msg_sender)));
    ...
    assertFalse(_put_ok, "path enc=31p1 exits through a REVERT: the call must fail on the unmodified contract");
  }


  
  function test_put_AssetTransfer_MarkInspected_path31p1(address p_msg_sender) public {
    _veriput_parameterized(p_msg_sender);
    
    { AssetTransferCovTest_AssetTransfer_MarkInspected_concrete31p1__basis_root__W _veriput_w = new AssetTransferCovTest_AssetTransfer_MarkInspected_concrete31p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract AssetTransferCovTest_AssetTransfer_MarkInspected_concrete31p1__basis_root__W is Test {
  AssetTransfer c0;
  function setUp() public {
    c0 = new AssetTransfer();
  }
