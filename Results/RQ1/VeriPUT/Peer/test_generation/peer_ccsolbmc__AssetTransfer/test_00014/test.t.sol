...
contract AssetTransferCovTest_AssetTransfer_ModifyOffer_put2p1 is Test {
  AssetTransfer c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 offerPrice) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("ModifyOffer(uint256)", offerPrice));
  ...
  function test_put_AssetTransfer_ModifyOffer_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 offerPrice) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, offerPrice);
  }
}
