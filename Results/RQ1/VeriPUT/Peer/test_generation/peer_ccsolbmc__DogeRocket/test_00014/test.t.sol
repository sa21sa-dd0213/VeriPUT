...
contract DogeRocketCovTest_DogeRocket_setCooldownEnabled_put7p1_p1_part_part0_r_w is Test {
  DogeRocket c0;
  ...
  function _veriput_parameterized(address p_msg_sender, bool onoff) internal {
    ...
    assertEq((uint256(vm.load(address(c0), bytes32(uint256(0)))) & 1461501637330902918203684832716283019655932542975), uint256(uint256(uint160(p_msg_sender))), "entry pin state._owner$157 did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    ...
    vm.assume(_pre_owner_157 == uint256(uint160(p_msg_sender)));
  ...
  function test_put_DogeRocket_setCooldownEnabled_path7p1_part_part0_r_w(address p_msg_sender, bool onoff) public {
    _veriput_parameterized(p_msg_sender, onoff);
    
    { DogeRocketCovTest_DogeRocket_setCooldownEnabled_concrete7p1__basis_part0_r_w__W _veriput_w = new DogeRocketCovTest_DogeRocket_setCooldownEnabled_concrete7p1__basis_part0_r_w__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract DogeRocketCovTest_DogeRocket_setCooldownEnabled_concrete7p1__basis_part0_r_w__W is Test {
  DogeRocket c0;
  function setUp() public {
    c0 = new DogeRocket();
  }
