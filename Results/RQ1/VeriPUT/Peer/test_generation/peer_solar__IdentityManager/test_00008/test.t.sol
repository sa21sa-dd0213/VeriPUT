...
contract IdentityManagerCovTest_IdentityManager_getIdentity_put3p1 is Test {
  IdentityManager c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address account) internal {
    ...
    uint256 _ret_pre_index_account = uint256(vm.load(address(c0), keccak256(abi.encode(account, uint256(1)))));
    ...
    uint256 _put_ret = c0.getIdentity(account);
    ...
    assertEq(uint256(_put_ret), _ret_pre_index_account, "return: return == state._index[account]");
    
  }


  
  function test_put_IdentityManager_getIdentity_path3p1(address p_msg_sender, address account) public {
    _veriput_parameterized(p_msg_sender, account);
    
    { IdentityManagerCovTest_IdentityManager_getIdentity_concrete3p1__basis_root__W _veriput_w = new IdentityManagerCovTest_IdentityManager_getIdentity_concrete3p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract IdentityManagerCovTest_IdentityManager_getIdentity_concrete3p1__basis_root__W is Test {
  IdentityManager c0;
  function setUp() public {
    c0 = new IdentityManager();
  }
