...
contract IdentityManagerCovTest_IdentityManager_convertAddress_put2p1 is Test {
  IdentityManager c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address account) internal {
    ...
    uint256 _pre_currentIndex = uint256(vm.load(address(c0), bytes32(uint256(2))));
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("convertAddress(address)", account));
    
    uint256 _post_currentIndex = uint256(vm.load(address(c0), bytes32(uint256(2))));
  ...
  function test_put_IdentityManager_convertAddress_path2p1(address p_msg_sender, uint256 p_msg_value, address account) public {
