...
contract DateTimeCovTest_DateTime_getDaysInMonth_put2046p1_p1_part_part0_w_w is Test {
  DateTime c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_block_timestamp, uint256 p_block_number, uint8 month, uint16 year) internal {
    ...
    if (p_msg_sender == address(uint160(0)) && p_block_timestamp == uint256(0) && p_block_number == uint256(0) && month == uint8(0) && year == uint16(0)) {
  ...
  function test_put_DateTime_getDaysInMonth_path2046p1_part_part0_w_w(address p_msg_sender, uint256 p_block_timestamp, uint256 p_block_number, uint8 month, uint16 year) public {
    _veriput_parameterized(p_msg_sender, p_block_timestamp, p_block_number, month, year);
    
    { DateTimeCovTest_DateTime_getDaysInMonth_concrete2046p1__basis_part0_w_w__W _veriput_w = new DateTimeCovTest_DateTime_getDaysInMonth_concrete2046p1__basis_part0_w_w__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_2(); }
}
}
...
contract DateTimeCovTest_DateTime_getDaysInMonth_concrete2046p1__basis_part0_w_w__W is Test {
  DateTime c0;
  function setUp() public {
    c0 = new DateTime();
  }
