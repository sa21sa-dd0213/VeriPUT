...
contract salvadorCovTest_salvador_balanceOf_put3p1 is Test {
  salvador c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address tokenOwner) internal {
    ...
    uint256 _ret_pre_balances_tokenOwner = uint256(vm.load(address(c0), keccak256(abi.encode(tokenOwner, uint256(6)))));
    ...
    uint256 _put_ret = c0.balanceOf(tokenOwner);
    
    if (p_msg_sender == address(uint160(0)) && tokenOwner == address(uint160(0))) {
    ...
    assertEq(uint256(_put_ret), _ret_pre_balances_tokenOwner, "return: return == state.balances[tokenOwner]");
    
  }


  
  function test_put_salvador_balanceOf_path3p1(address p_msg_sender, address tokenOwner) public {
    _veriput_parameterized(p_msg_sender, tokenOwner);
    
    { salvadorCovTest_salvador_balanceOf_concrete3p1__basis_root__W _veriput_w = new salvadorCovTest_salvador_balanceOf_concrete3p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract salvadorCovTest_salvador_balanceOf_concrete3p1__basis_root__W is Test {
  salvador c0;
  function setUp() public {
    c0 = new salvador();
  }
