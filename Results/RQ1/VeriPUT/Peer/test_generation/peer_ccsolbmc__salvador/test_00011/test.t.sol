...
contract salvadorCovTest_salvador_transfer_put2p1 is Test {
  salvador c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address to, uint256 tokens) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("transfer(address,uint256)", to, tokens));
  ...
  function test_put_salvador_transfer_path2p1(address p_msg_sender, uint256 p_msg_value, address to, uint256 tokens) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, to, tokens);
  }
}
