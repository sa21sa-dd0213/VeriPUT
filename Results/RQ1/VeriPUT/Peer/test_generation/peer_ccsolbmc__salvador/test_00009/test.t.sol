...
contract salvadorCovTest_salvador_sdfd_put3p1_p1_part_part0_w_r is Test {
  ...
  //   msg.sender in [2147483649, 4294967294]
  //   PIN block.basefee == 0
  ...
  //   rung IMPLIED (not lost, not asserted twice): admin: post != pre (entailed by a stronger rung that also HOLDS on admin, so asserting it detects nothing the stronger one misses)
  //   environment ESTABLISHED: msg.sender in [2147483649, 4294967294] is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it (replacing `vm.prank(address(uint160(0)));`)
