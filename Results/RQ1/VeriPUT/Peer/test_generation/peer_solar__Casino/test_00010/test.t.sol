...
contract CasinoCovTest_Casino_generateHash_put2p1 is Test {
  Casino c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 number, uint256 salt) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("generateHash(uint256,uint256)", number, salt));
  ...
  function test_put_Casino_generateHash_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 number, uint256 salt) public {
