...
contract AnyswapV5ERC20CovTest_AnyswapV5ERC20_Swapout_put2p1 is Test {
  AnyswapV5ERC20 c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 amount, address bindaddr) internal {
    ...
    bindaddr = address(uint160(bound(uint256(uint160(bindaddr)), 0, 1461501637330902918203684832716283019655932542975)));
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("Swapout(uint256,address)", amount, bindaddr));
  ...
  function test_put_AnyswapV5ERC20_Swapout_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 amount, address bindaddr) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, amount, bindaddr);
  }
}
