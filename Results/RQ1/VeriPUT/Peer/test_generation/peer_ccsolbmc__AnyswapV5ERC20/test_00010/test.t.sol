...
contract AnyswapV5ERC20CovTest_AnyswapV5ERC20_revokeMinter_put2p1 is Test {
  AnyswapV5ERC20 c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address _auth) internal {
    ...
    uint256 _pre_isMinter__auth = (uint256(vm.load(address(c0), keccak256(abi.encode(_auth, uint256(8))))) & 255);
    
    
    vm.deal(p_msg_sender, p_msg_value);
    vm.prank(p_msg_sender);
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("revokeMinter(address)", _auth));
    ...
    uint256 _post_isMinter__auth = (uint256(vm.load(address(c0), keccak256(abi.encode(_auth, uint256(8))))) & 255);
    ...
    assertEq(_post_isMinter__auth, _pre_isMinter__auth, "isMinter[_auth]: post == pre");
  ...
  function test_put_AnyswapV5ERC20_revokeMinter_path2p1(address p_msg_sender, uint256 p_msg_value, address _auth) public {
