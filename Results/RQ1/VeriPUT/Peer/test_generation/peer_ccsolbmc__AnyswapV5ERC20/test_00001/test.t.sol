...
contract AnyswapV5ERC20CovTest_AnyswapV5ERC20_Swapout_put14p1 is Test {
  ...
  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@AnyswapV5ERC20@F@Swapout#726:path:14   depth=3
  ...
  //   environment ESTABLISHED: msg.sender in [0, 1099511627776]  \ {0} is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it, and the 1 punched value(s) are excluded by vm.assume; 0 is excluded because Foundry cannot prank address(0), and the remaining executable subregion is a subset of the certified region (no prank was present, so one was inserted)
