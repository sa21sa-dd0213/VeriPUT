...
contract B6CovTest_B6_g6_put7p1_p1_part_part0_r_w is Test {
  ...
  //   environment ESTABLISHED: msg.sender in [47, 2147483686] is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it (no prank was present, so one was inserted)
