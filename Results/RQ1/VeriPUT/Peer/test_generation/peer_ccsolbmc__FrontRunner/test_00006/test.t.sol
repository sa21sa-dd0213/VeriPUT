...
import {FrontRunner, ERC20} from "../src/flat.sol";

contract FrontRunnerCovTest_FrontRunner_drainToken_put2p1 is Test {
  FrontRunner c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address _token) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("drainToken(ERC20)", ERC20(_token)));
  ...
  function test_put_FrontRunner_drainToken_path2p1(address p_msg_sender, uint256 p_msg_value, address _token) public {
