...
contract FrontRunnerCovTest_0_FrontRunner_kill_put7p1_p1_part_part0_w_w is Test {
  ...
  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@FrontRunner@F@kill#599:path:7   depth=2
  ...
  //   rung IMPLIED (not lost, not asserted twice): manager: post >= pre (entailed by a stronger rung that also HOLDS on manager, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): manager: post <= pre (entailed by a stronger rung that also HOLDS on manager, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): EOA1: post >= pre (entailed by a stronger rung that also HOLDS on EOA1, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): EOA1: post <= pre (entailed by a stronger rung that also HOLDS on EOA1, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): manager: post == (pre + msg.value) (entailed by a stronger rung that also HOLDS on manager, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): EOA1: post == (pre + msg.value) (entailed by a stronger rung that also HOLDS on EOA1, so asserting it detects nothing the stronger one misses)
