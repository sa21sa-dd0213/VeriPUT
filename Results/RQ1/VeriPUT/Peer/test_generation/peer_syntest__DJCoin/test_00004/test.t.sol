...
contract DJCoinCovTest_DJCoin_approve_put3p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, address spender, uint256 value) internal {
    ...
    if (p_msg_sender == address(uint160(0)) && spender == address(uint160(0)) && value == uint256(0)) {
      ...
      assertEq(uint256(vm.load(address(c0), keccak256(abi.encode(address(uint160(0)), keccak256(abi.encode(address(uint160(0)), uint256(4))))))), uint256(0), "fixed witness state");
