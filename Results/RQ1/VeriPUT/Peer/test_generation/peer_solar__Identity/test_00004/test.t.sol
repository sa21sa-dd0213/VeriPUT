...
contract IdentityCovTest_Identity_addPermission_put2p1 is Test {
  Identity c0;
  function setUp() public {
    c0 = new Identity(address(uint160(1000)));
  }
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address caller) internal {
    ...
    uint256 _pre_approvedCallers_msg_sender = (uint256(vm.load(address(c0), keccak256(abi.encode(p_msg_sender, uint256(0))))) & 255);
    
    
    vm.deal(p_msg_sender, p_msg_value);
    vm.prank(p_msg_sender);
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("addPermission(address)", caller));
    ...
    uint256 _post_approvedCallers_msg_sender = (uint256(vm.load(address(c0), keccak256(abi.encode(p_msg_sender, uint256(0))))) & 255);
  ...
  function test_put_Identity_addPermission_path2p1(address p_msg_sender, uint256 p_msg_value, address caller) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, caller);
  }
}
