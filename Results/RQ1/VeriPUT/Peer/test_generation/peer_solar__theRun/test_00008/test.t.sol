...
contract theRunCovTest_theRun_WatchBalance_put3p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender) internal {
    ...
    assertEq(uint256(_put_ret), (uint256(_ret_pre_Balance) - uint256(0)), "return: return == (state.Balance - msg.value)");
    assertEq(uint256(_put_ret), (uint256(_ret_pre_Balance) + uint256(0)), "return: return == (state.Balance + 0)");
    assertEq(uint256(_put_ret), (uint256(_ret_pre_Balance) - uint256(0)), "return: return == (state.Balance - 0)");
    assertEq(uint256(_put_ret), (uint256(0) + uint256(_ret_pre_Balance)), "return: return == (0 + state.Balance)");
    assertGe(uint256(_put_ret), _ret_pre_Balance, "return: return in [state.Balance, state.Balance]");
    assertLe(uint256(_put_ret), _ret_pre_Balance, "return: return in [state.Balance, state.Balance]");
