...
contract RIASCovTest_RIAS_setBlackAddress_put2p1 is Test {
  RIAS c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address blackAddress) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("setBlackAddress(address)", blackAddress));
  ...
  function test_put_RIAS_setBlackAddress_path2p1(address p_msg_sender, uint256 p_msg_value, address blackAddress) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, blackAddress);
  }
}
