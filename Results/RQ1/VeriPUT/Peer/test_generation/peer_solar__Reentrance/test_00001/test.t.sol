...
contract ReentranceCovTest_Reentrance_balanceOf_put2p1 is Test {
  Reentrance c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address _who) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("balanceOf(address)", _who));
  ...
  function test_put_Reentrance_balanceOf_path2p1(address p_msg_sender, uint256 p_msg_value, address _who) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, _who);
  }
}
