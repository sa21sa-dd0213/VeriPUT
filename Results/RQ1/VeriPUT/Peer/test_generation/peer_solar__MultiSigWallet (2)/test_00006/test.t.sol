...
contract MultiSigWalletCovTest_MultiSigWallet_signTransaction_put2p1 is Test {
  MultiSigWallet c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 transactionId) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("signTransaction(uint256)", transactionId));
  ...
  function test_put_MultiSigWallet_signTransaction_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 transactionId) public {
