...
contract MultiSigWalletCovTest_MultiSigWallet_removeOwner_put2p1 is Test {
  MultiSigWallet c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address owner) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("removeOwner(address)", owner));
    ...
    assertGe(_post_owners_owner, _pre_owners_owner, "_owners[owner]: post >= pre");
    assertLe(_post_owners_owner, _pre_owners_owner, "_owners[owner]: post <= pre");
  ...
  function test_put_MultiSigWallet_removeOwner_path2p1(address p_msg_sender, uint256 p_msg_value, address owner) public {
