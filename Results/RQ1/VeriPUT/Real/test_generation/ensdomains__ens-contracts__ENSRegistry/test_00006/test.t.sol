...
contract ENSRegistryCovTest_ENSRegistry_resolver_put3p1 is Test {
  ENSRegistry c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 node) internal {
    ...
    uint256 _ret_pre_records_node__resolver = (uint256(vm.load(address(c0), bytes32(uint256(keccak256(abi.encode(bytes32(node), uint256(0)))) + 1))) & 1461501637330902918203684832716283019655932542975);
    ...
    address _put_ret = c0.resolver(bytes32(node));
    ...
    assertEq(uint256(uint160(_put_ret)), _ret_pre_records_node__resolver, "return: return == state.records[node].resolver");
    
  }


  
  function test_put_ENSRegistry_resolver_path3p1(address p_msg_sender, uint256 node) public {
    _veriput_parameterized(p_msg_sender, node);
    
    { ENSRegistryCovTest_ENSRegistry_resolver_concrete3p1__basis_root__W _veriput_w = new ENSRegistryCovTest_ENSRegistry_resolver_concrete3p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract ENSRegistryCovTest_ENSRegistry_resolver_concrete3p1__basis_root__W is Test {
  ENSRegistry c0;
  function setUp() public {
    vm.startPrank(address(uint160(1)), address(uint160(1)));
    c0 = new ENSRegistry();
