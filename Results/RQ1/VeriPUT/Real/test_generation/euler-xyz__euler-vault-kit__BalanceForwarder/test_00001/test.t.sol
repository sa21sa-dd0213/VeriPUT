...
import {BalanceForwarder, Base} from "../src/flat.sol";

contract BalanceForwarderCovTest_BalanceForwarder_balanceForwarderEnabled_put2p1 is Test {
  BalanceForwarder c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address account) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("balanceForwarderEnabled(address)", account));
  ...
  function test_put_BalanceForwarder_balanceForwarderEnabled_path2p1(address p_msg_sender, uint256 p_msg_value, address account) public {
