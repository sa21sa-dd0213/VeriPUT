...
import {CowRouter, IVault, IAuthorizer, IERC20, PoolRoleAccounts, TokenInfo} from "../src/flat.sol";

contract CowRouterCovTest_CowRouter_getActionId_put3p1 is Test {
  CowRouter c0;
  function setUp() public {
    ...
    address _esbmc_ctor_state_mock_0_28 = address(IVault(address(uint160(1000))));
  ...
  function _veriput_parameterized(address p_msg_sender, uint32 selector) internal {
    ...
    selector = uint32(bound(uint256(selector), 0, 6981463658332));
  ...
  function test_put_CowRouter_getActionId_path3p1(address p_msg_sender, uint32 selector) public {
