...
contract TimelockAuthorizerCovTest_TimelockAuthorizer_getActionIdRevokeDelay_put3p1 is Test {
  ...
  // claim: sol:@C@TimelockAuthorizer@F@getActionIdRevokeDelay#1913:path:3

  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@TimelockAuthorizer@F@getActionIdRevokeDelay#1913:path:3   depth=1
  ...
  //   rung IMPLIED (not lost, not asserted twice): _revokeDelays[actionId]: post >= pre (entailed by a stronger rung that also HOLDS on _revokeDelays[actionId], so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): _revokeDelays[actionId]: post <= pre (entailed by a stronger rung that also HOLDS on _revokeDelays[actionId], so asserting it detects nothing the stronger one misses)
