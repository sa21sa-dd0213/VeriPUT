...
contract ProtocolConfigCovTest_ProtocolConfig_setFeeReceiver_put2p1 is Test {
  ProtocolConfig c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address newReceiver) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("setFeeReceiver(address)", newReceiver));
  ...
  function test_put_ProtocolConfig_setFeeReceiver_path2p1(address p_msg_sender, uint256 p_msg_value, address newReceiver) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, newReceiver);
  }
}
