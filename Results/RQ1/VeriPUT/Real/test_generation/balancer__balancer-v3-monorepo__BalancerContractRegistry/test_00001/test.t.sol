...
contract BalancerContractRegistryCovTest_BalancerContractRegistry_addOrUpdateBalancerContractAlias_put2p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, string memory contractAlias, address contractAddress) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("addOrUpdateBalancerContractAlias(string,address)", contractAlias, contractAddress));
  ...
  function test_put_BalancerContractRegistry_addOrUpdateBalancerContractAlias_path2p1(address p_msg_sender, uint256 p_msg_value, string memory contractAlias, address contractAddress) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, contractAlias, contractAddress);
  }
}
