...
contract BalancerContractRegistryCovTest_BalancerContractRegistry_registerBalancerContract_put2p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, string memory contractName, address contractAddress) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("registerBalancerContract(ContractType,string,address)", ContractType(0), contractName, contractAddress));
  ...
  function test_put_BalancerContractRegistry_registerBalancerContract_path2p1(address p_msg_sender, uint256 p_msg_value, string memory contractName, address contractAddress) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, contractName, contractAddress);
  }
}
