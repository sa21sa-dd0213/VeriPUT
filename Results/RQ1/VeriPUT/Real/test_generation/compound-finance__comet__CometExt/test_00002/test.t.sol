...
contract CometExtCovTest_CometExt_allowance_put2p1 is Test {
  CometExt c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address owner, address spender) internal {
    ...
    uint256 _pre_isAllowed_owner__spender = (uint256(vm.load(address(c0), keccak256(abi.encode(spender, keccak256(abi.encode(owner, uint256(3))))))) & 255);
    ...
    uint256 _post_isAllowed_owner__spender = (uint256(vm.load(address(c0), keccak256(abi.encode(spender, keccak256(abi.encode(owner, uint256(3))))))) & 255);
    assertEq(_post_isAllowed_owner__spender, _pre_isAllowed_owner__spender, "isAllowed[owner][spender]: post == pre");
  ...
  function test_put_CometExt_allowance_path2p1(address p_msg_sender, uint256 p_msg_value, address owner, address spender) public {
