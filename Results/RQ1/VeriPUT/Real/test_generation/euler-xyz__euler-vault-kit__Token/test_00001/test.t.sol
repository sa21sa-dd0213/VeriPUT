...
contract TokenCovTest_Token_allowance_put2p1 is Test {
  Token c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address holder, address spender) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("allowance(address,address)", holder, spender));
  ...
  function test_put_Token_allowance_path2p1(address p_msg_sender, uint256 p_msg_value, address holder, address spender) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, holder, spender);
  }
}
