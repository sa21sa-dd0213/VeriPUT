...
contract TokenCovTest_Token_balanceOf_put7p1 is Test {
  Token c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address account, uint8 s_initialized_469) internal {
    ...
    s_initialized_469 = uint8(bound(uint256(s_initialized_469), 0, 1));
    
    
    {
      uint256 _w = uint256(vm.load(address(c0), bytes32(uint256(0))));
      _w = (_w & ~uint256(255)) | ((uint256(uint256(s_initialized_469)) & 255) << 0);
      vm.store(address(c0), bytes32(uint256(0)), bytes32(_w));
    }
    assertLe((uint256(vm.load(address(c0), bytes32(uint256(0)))) & 255), uint256(1), "the entry state is OUTSIDE the certified region: state.initialized$469 was assumed in [0, 1] when the path was certified, so a value outside it means the assumption was vacuous and the rungs below were proved about no execution this test can reach");
    ...
    if (p_msg_sender == address(uint160(0)) && account == address(uint160(0)) && s_initialized_469 == uint8(0)) {
  ...
  function test_put_Token_balanceOf_path7p1(address p_msg_sender, address account, uint8 s_initialized_469) public {
    _veriput_parameterized(p_msg_sender, account, s_initialized_469);
    
    { TokenCovTest_Token_balanceOf_concrete7p1__basis_root__W _veriput_w = new TokenCovTest_Token_balanceOf_concrete7p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract TokenCovTest_Token_balanceOf_concrete7p1__basis_root__W is Test {
  Token c0;
  function setUp() public {
    Base.Integrations memory _esbmc_ctor_code_struct_0_0 = Base.Integrations(address(uint160(0)), address(uint160(0)), address(uint160(0)), address(uint160(0)), address(uint160(0)));
  ...
  function _w_test_cov_0() public {
    ...
    assertEq((uint256(vm.load(address(c0), bytes32(uint256(0)))) & 255), uint256(0), "entry pin state.initialized$469 did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    ...
    uint256 _veriput_fixed_state_initialized_469_0 = (uint256(vm.load(address(c0), bytes32(uint256(0)))) & 255);
    assertEq(_veriput_fixed_state_initialized_469_0, uint256(0), "fixed witness state");
  }
