...
contract PolygonBridgeReceiverCovTest_2_PolygonBridgeReceiver_processMessageFromRoot_put6p1 is Test {
  ...
  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@PolygonBridgeReceiver@F@processMessageFromRoot#535:path:6   depth=2
  ...
  //   rootMessageSender in [0, 1461501637330902918203684832716283019655932542975]
  //   state.fxChild in [730750818665451459101842416358141509832261238783, 730750818665451459101842416358141509832261238783]
  //   state.govTimelock$57 in [0, 1461501637330902918203684832716283019655932542975]
  ...
  //   environment ESTABLISHED: msg.sender in [0, 1461501637330902918203684832716283019655932542975]  \ {0, 730750818665451459101842416358141509832261238783} is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it, and the 2 punched value(s) are excluded by vm.assume; 0 is excluded because Foundry cannot prank address(0), and the remaining executable subregion is a subset of the certified region (no prank was present, so one was inserted)
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 stateId, address rootMessageSender, bytes memory data) internal {
    ...
    //   state.fxChild := 730750818665451459101842416358141509832261238783
    {
