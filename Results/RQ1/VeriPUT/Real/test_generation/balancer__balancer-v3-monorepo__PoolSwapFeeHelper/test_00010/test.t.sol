...
import {PoolSwapFeeHelper, IVault, IAuthorizer, PoolRoleAccounts} from "../src/flat.sol";

contract PoolSwapFeeHelperCovTest_PoolSwapFeeHelper_pendingOwner_put3p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender) internal {
    ...
    uint256 _pre_pendingOwner_428 = (uint256(vm.load(address(c0), bytes32(uint256(1)))) & 1461501637330902918203684832716283019655932542975);
  ...
  function test_put_PoolSwapFeeHelper_pendingOwner_path3p1(address p_msg_sender) public {
    _veriput_parameterized(p_msg_sender);
    
    { PoolSwapFeeHelperCovTest_PoolSwapFeeHelper_pendingOwner_concrete3p1__basis_root__W _veriput_w = new PoolSwapFeeHelperCovTest_PoolSwapFeeHelper_pendingOwner_concrete3p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}



contract PoolSwapFeeHelperCovTest_PoolSwapFeeHelper_pendingOwner_concrete3p1__basis_root__W is Test {
  PoolSwapFeeHelper c0;
  function setUp() public {
    c0 = new PoolSwapFeeHelper(IVault(address(uint160(1000))), address(uint160(1001)));
