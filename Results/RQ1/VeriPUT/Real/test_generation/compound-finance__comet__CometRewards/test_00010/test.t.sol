...
contract CometRewardsCovTest_CometRewards_transferGovernor_put2p1 is Test {
  CometRewards c0;
  function setUp() public {
    c0 = new CometRewards(address(uint160(1000)));
  }
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address newGovernor) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("transferGovernor(address)", newGovernor));
  ...
  function test_put_CometRewards_transferGovernor_path2p1(address p_msg_sender, uint256 p_msg_value, address newGovernor) public {
