...
contract MigrationHelperCovTest_MigrationHelper_setMigrationTarget_put2p1 is Test {
  MigrationHelper c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address target) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("setMigrationTarget(address)", target));
  ...
  function test_put_MigrationHelper_setMigrationTarget_path2p1(address p_msg_sender, uint256 p_msg_value, address target) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, target);
  }
}
