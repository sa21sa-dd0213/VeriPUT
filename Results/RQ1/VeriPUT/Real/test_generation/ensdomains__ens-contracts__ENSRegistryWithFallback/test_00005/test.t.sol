...
contract ENSRegistryWithFallbackCovTest_ENSRegistryWithFallback_recordExists_put2p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 node) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("recordExists(bytes32)", bytes32(node)));
  ...
  function test_put_ENSRegistryWithFallback_recordExists_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 node) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, node);
  }
}
