...
contract ENSRegistryWithFallbackCovTest_ENSRegistryWithFallback_setOwner_put2p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 node, address owner) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("setOwner(bytes32,address)", bytes32(node), owner));
  ...
  function test_put_ENSRegistryWithFallback_setOwner_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 node, address owner) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, node, owner);
  }
}
