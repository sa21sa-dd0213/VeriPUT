...
contract ProtocolFeeControllerCovTest_ProtocolFeeController_getPoolProtocolSwapFeeInfo_put2p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address pool) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("getPoolProtocolSwapFeeInfo(address)", pool));
  ...
  function test_put_ProtocolFeeController_getPoolProtocolSwapFeeInfo_path2p1(address p_msg_sender, uint256 p_msg_value, address pool) public {
