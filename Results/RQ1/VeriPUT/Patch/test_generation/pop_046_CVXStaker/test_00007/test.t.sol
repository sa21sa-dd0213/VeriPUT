...
contract CVXStakerCovTest_CVXStaker_isCvxShutdown_put2p1 is Test {
  CVXStaker c0;
  function setUp() public {
    ...
    vm.mockCall(_esbmc_ctor_state_mock_0_0, abi.encodeWithSignature("poolInfo(uint256)"), abi.encode(ICVXBooster.PoolInfo({lptoken: address(0), token: address(0), gauge: address(0), crvRewards: address(0), stash: address(0), shutdown: false})));
  }
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("isCvxShutdown()"));
  ...
  function test_put_CVXStaker_isCvxShutdown_path2p1(address p_msg_sender, uint256 p_msg_value) public {
