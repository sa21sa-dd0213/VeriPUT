...
contract NewIntelTechMediaCovTest_0_NewIntelTechMedia_NETM_put7p1 is Test {
  ...
  // claim: sol:@C@NewIntelTechMedia@F@NETM#313:path:7
  // witness-fingerprint-sha256: 57be111874aa57235478c90b880272384f2346ba70f9eaabced59d139ce9d864
  // claim: sol:@C@NewIntelTechMedia@F@NETM#313:path:2
  // witness-fingerprint-sha256: 39fb6317f7a44c57e6261ef3cf7f3ca0bf5482d3d91282a51e1a41453d506e5f

  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@NewIntelTechMedia@F@NETM#313:path:7   depth=2
  ...
  //   state.totalDistributed in [0, 115792089237316195423570985008687907853269984665640564039457584007913129639935]
  //   PIN block.basefee == 0
  ...
  //   rung IMPLIED (not lost, not asserted twice): owner: post <= pre (entailed by a stronger rung that also HOLDS on owner, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): totalDistributed: post >= pre (entailed by a stronger rung that also HOLDS on totalDistributed, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): totalDistributed: post <= pre (entailed by a stronger rung that also HOLDS on totalDistributed, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): totalDistributed: post == (pre + msg.value) (entailed by a stronger rung that also HOLDS on totalDistributed, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): totalDistributed: post in [state.totalDistributed, state.totalDistributed] (entailed by a stronger rung that also HOLDS on totalDistributed, so asserting it detects nothing the stronger one misses)
...
contract NewIntelTechMediaCovTest_1 is Test {
  ...
  // claim: sol:@C@NewIntelTechMedia@F@NETM#313:path:6
  // witness-fingerprint-sha256: 12d42ca33c3548ca98d5e20d891f2ef3c2799e0085609c260d267f91a77e7ae4
}
