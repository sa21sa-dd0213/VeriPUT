...
contract NewIntelTechMediaCovTest_1_NewIntelTechMedia_withdrawForeignTokens_put6p1_p1_part_part0_r_w is Test {
  NewIntelTechMedia c0;
  function setUp() public {
  ...
  function _veriput_parameterized(address p_msg_sender, address _tokenContract) internal {
    p_msg_sender = address(uint160(bound(uint256(uint160(p_msg_sender)), 1073741824, 2147483646)));
    _tokenContract = address(uint160(bound(uint256(uint160(_tokenContract)), 0, 1461501637330902918203684832716283019655932542975)));
    ...
    bool _put_ok = true;
    try c0.withdrawForeignTokens(_tokenContract) {} catch { _put_ok = false; }
    
    if (p_msg_sender == address(uint160(2147483646)) && _tokenContract == address(uint160(0))) {
  ...
  function test_put_NewIntelTechMedia_withdrawForeignTokens_path6p1_part_part0_r_w(address p_msg_sender, address _tokenContract) public {
    _veriput_parameterized(p_msg_sender, _tokenContract);
    
    { NewIntelTechMediaCovTest_1_NewIntelTechMedia_withdrawForeignTokens_concrete6p1__basis_part0_r_w__W _veriput_w = new NewIntelTechMediaCovTest_1_NewIntelTechMedia_withdrawForeignTokens_concrete6p1__basis_part0_r_w__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_2(); }
}
}
...
contract NewIntelTechMediaCovTest_1_NewIntelTechMedia_withdrawForeignTokens_concrete6p1__basis_part0_r_w__W is Test {
  NewIntelTechMedia c0;
  function setUp() public {
    vm.startPrank(address(uint160(2147483647)), address(uint160(2147483647)));
    c0 = new NewIntelTechMedia();
  ...
  function _w_test_cov_2() public {
    ...
    vm.txGasPrice(0);
    vm.coinbase(address(uint160(0)));
    vm.prank(address(uint160(2147483646)), address(uint160(0)));
    bool _veriput_concrete_completed = false;
    try c0.withdrawForeignTokens(address(uint160(0))) {
