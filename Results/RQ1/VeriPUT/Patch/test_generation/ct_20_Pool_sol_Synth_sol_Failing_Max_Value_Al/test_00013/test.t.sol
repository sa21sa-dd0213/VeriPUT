...
contract SynthCovTest_Synth_transfer_put14p1_p1_part_part0_r is Test {
  Synth c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address recipient, uint256 amount) internal {
    ...
    if (p_msg_sender == address(uint160(1)) && recipient == address(uint160(0)) && amount == uint256(1)) {
  ...
  function test_put_Synth_transfer_path14p1_part_part0_r(address p_msg_sender, address recipient, uint256 amount) public {
    _veriput_parameterized(p_msg_sender, recipient, amount);
    
    { SynthCovTest_Synth_transfer_concrete14p1__basis_part0_r__W _veriput_w = new SynthCovTest_Synth_transfer_concrete14p1__basis_part0_r__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_1(); }
}
}
...
contract SynthCovTest_Synth_transfer_concrete14p1__basis_part0_r__W is Test {
  Synth c0;
  function setUp() public {
    vm.warp(1);
    vm.startPrank(address(uint160(1)), address(uint160(1)));
