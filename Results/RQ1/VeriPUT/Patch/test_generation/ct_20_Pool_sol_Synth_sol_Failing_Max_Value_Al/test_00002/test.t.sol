...
contract SynthCovTest_Synth_allowance_put3p1 is Test {
  Synth c0;
  ...
  function _veriput_parameterized(address p_msg_sender, address owner, address spender) internal {
    ...
    uint256 _ret_pre_allowances_owner__spender = uint256(vm.load(address(c0), keccak256(abi.encode(spender, keccak256(abi.encode(owner, uint256(9)))))));
  ...
  function test_put_Synth_allowance_path3p1(address p_msg_sender, address owner, address spender) public {
    _veriput_parameterized(p_msg_sender, owner, spender);
    
    { SynthCovTest_Synth_allowance_concrete3p1__basis_root__W _veriput_w = new SynthCovTest_Synth_allowance_concrete3p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract SynthCovTest_Synth_allowance_concrete3p1__basis_root__W is Test {
  Synth c0;
  function setUp() public {
    vm.warp(1);
    vm.startPrank(address(uint160(1)), address(uint160(1)));
