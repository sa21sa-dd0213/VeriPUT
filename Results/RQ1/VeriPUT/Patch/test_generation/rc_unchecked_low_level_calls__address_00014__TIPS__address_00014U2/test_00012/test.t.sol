...
contract PoCGameCovTest_PoCGame_currentBetLimit_put3p1 is Test {
  ...
  // claim: sol:@C@PoCGame@F@currentBetLimit#416:path:3
  // witness-fingerprint-sha256: d9de2a0739d5f59e54891af5ca0d8145870f60dacea732b783ee5d12007ba122
  // claim: sol:@C@PoCGame@F@currentBetLimit#416:path:2
  // witness-fingerprint-sha256: d3c30ad489ff6435d4dcb0677ccf35399c25799372e90ad9906595c7aca058a2

  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@PoCGame@F@currentBetLimit#416:path:3   depth=1
  ...
  //   rung IMPLIED (not lost, not asserted twice): betLimit: post >= pre (entailed by a stronger rung that also HOLDS on betLimit, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): betLimit: post <= pre (entailed by a stronger rung that also HOLDS on betLimit, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): betLimit: post == (pre + msg.value) (entailed by a stronger rung that also HOLDS on betLimit, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): betLimit: post in [state.betLimit, state.betLimit] (entailed by a stronger rung that also HOLDS on betLimit, so asserting it detects nothing the stronger one misses)
