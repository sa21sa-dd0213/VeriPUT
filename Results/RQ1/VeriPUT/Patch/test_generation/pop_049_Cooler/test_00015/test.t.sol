...
contract CoolerCovTest_Cooler_transferOwnership_put2p1 is Test {
  Cooler c0;
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 loanID_) internal {
    ...
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("transferOwnership(uint256)", loanID_));
  ...
  function test_put_Cooler_transferOwnership_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 loanID_) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, loanID_);
  }
}
