...
contract GameItemsCovTest_1_GameItems_transferOwnership_put6p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, address newOwnerAddress) internal {
    ...
    if (p_msg_sender == address(uint160(4294967295)) && newOwnerAddress == address(uint160(0))) {
      ...
      assertEq((uint256(vm.load(address(c0), bytes32(uint256(7)))) & 1461501637330902918203684832716283019655932542975), uint256(730750818665451459101842416358141509832261238783), "fixed witness state");
...
contract GameItemsCovTest_1_GameItems_transferOwnership_concrete6p1__basis_root__W is Test {
  ...
  function _w_test_cov_2() public {
    ...
    assertEq((uint256(vm.load(address(c0), bytes32(uint256(7)))) & 1461501637330902918203684832716283019655932542975), uint256(730750818665451459101842416358141509832261238783), "entry pin state._ownerAddress$3365 did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    {
      ...
      vm.store(address(c0), keccak256(abi.encode(uint256(730750818665451459101842416358141509832261238783), uint256(13))), bytes32(_w));
    }
    assertEq((uint256(vm.load(address(c0), keccak256(abi.encode(uint256(730750818665451459101842416358141509832261238783), uint256(13))))) & 255), uint256(1), "entry pin state.isAdmin$3391[(&_ESBMC_Object_GameItems)->_ownerAddress$3365] did NOT land: vm.store wrote a word the contract does not read back at this slot, so the test is not inside the certified region and every rung below is about a different state");
    {
    ...
    assertEq(_veriput_fixed_state_ownerAddress_3365_1, uint256(730750818665451459101842416358141509832261238783), "fixed witness state");
