...
contract TimelockControllerCovTest_TimelockController_getTimestamp_put3p1 is Test {
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 id) internal {
    ...
    uint256 _ret_pre_timestamps_id = uint256(vm.load(address(c0), keccak256(abi.encode(bytes32(id), uint256(1)))));
    ...
    uint256 _put_ret = c0.getTimestamp(bytes32(id));
    ...
    assertEq(uint256(_put_ret), _ret_pre_timestamps_id, "return: return == state._timestamps[id]");
    
  }


  
  function test_put_TimelockController_getTimestamp_path3p1(address p_msg_sender, uint256 id) public {
    _veriput_parameterized(p_msg_sender, id);
    
    { TimelockControllerCovTest_TimelockController_getTimestamp_concrete3p1__basis_root__W _veriput_w = new TimelockControllerCovTest_TimelockController_getTimestamp_concrete3p1__basis_root__W(); _veriput_w.setUp(); _veriput_w._w_test_cov_0(); }
}
}
...
contract TimelockControllerCovTest_TimelockController_getTimestamp_concrete3p1__basis_root__W is Test {
  TimelockController c0;
  function setUp() public {
    vm.startPrank(address(uint160(1)), address(uint160(1)));
    c0 = new TimelockController(0, new address[](4), new address[](4));
  ...
  function _w_test_cov_0() public {
    ...
    uint256 _veriput_concrete_return = c0.getTimestamp(bytes32(uint256(0)));
