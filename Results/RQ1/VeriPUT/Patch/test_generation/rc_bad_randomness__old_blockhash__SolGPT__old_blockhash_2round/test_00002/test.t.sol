...
contract PredictTheBlockHashChallengeCovTest_PredictTheBlockHashChallenge_lockInGuess_put7p1 is Test {
  ...
  // ===================== PUT (stage 4) =====================
  // claim: sol:@C@PredictTheBlockHashChallenge@F@lockInGuess#64:path:7   depth=2
  ...
  // FUZZ COORDINATES: msg.sender, block.number, hash
  ...
  //   rung IMPLIED (not lost, not asserted twice): guesses$11[msg.sender].block: post != pre (entailed by a stronger rung that also HOLDS on guesses$11[msg.sender].block, so asserting it detects nothing the stronger one misses)
  //   rung IMPLIED (not lost, not asserted twice): guesses$11[msg.sender].block: post >= pre (entailed by a stronger rung that also HOLDS on guesses$11[msg.sender].block, so asserting it detects nothing the stronger one misses)
  //   path guard DROPPED: '!(msg.value == return_value$__ESBMC_ether$1)' (`return_value$__ESBMC_ether$1` is not nameable in this PUT)
  //   environment ESTABLISHED: msg.sender in [1, 1461501637330902918203684832716283019655932542975] is ESTABLISHED and FUZZED: the governing vm.prank now takes the bound() fuzz parameter `p_msg_sender`, so this PUT ranges over the certified sender interval rather than over one point of it (and funded with `vm.deal(p_msg_sender, 1000000000000000000)`, because the call sends value and the sender pays) (replacing `vm.prank(address(uint160(0)));`)
