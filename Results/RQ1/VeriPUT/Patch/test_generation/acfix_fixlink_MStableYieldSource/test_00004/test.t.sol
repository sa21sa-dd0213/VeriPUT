...
contract MStableYieldSourceCovTest_MStableYieldSource_redeemToken_put2p1 is Test {
  ...
  function setUp() public {
    ...
    vm.mockCall(_esbmc_ctor_state_mock_0_0, abi.encodeWithSignature("redeemUnderlying(uint256)"), abi.encode(uint256(0)));
    vm.etch(_esbmc_ctor_arg_mock_0_0_ret_0, hex"60006000f3");
  }
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, uint256 mAssetAmount) internal {
    ...
    uint256 _pre_imBalances_msg_sender = uint256(vm.load(address(c0), keccak256(abi.encode(p_msg_sender, uint256(2)))));
    
    
    vm.deal(p_msg_sender, p_msg_value);
    vm.prank(p_msg_sender);
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("redeemToken(uint256)", mAssetAmount));
    
    uint256 _post_status = uint256(vm.load(address(c0), bytes32(uint256(0))));
    uint256 _post_imBalances_msg_sender = uint256(vm.load(address(c0), keccak256(abi.encode(p_msg_sender, uint256(2)))));
    assertEq(_post_status, _pre_status, "_status: post == pre");
    assertEq(_post_imBalances_msg_sender, _pre_imBalances_msg_sender, "imBalances[msg.sender]: post == pre");
    assertGe(_post_status, _pre_status, "_status: post >= pre");
    assertLe(_post_status, _pre_status, "_status: post <= pre");
    assertGe(_post_imBalances_msg_sender, _pre_imBalances_msg_sender, "imBalances[msg.sender]: post >= pre");
    assertLe(_post_imBalances_msg_sender, _pre_imBalances_msg_sender, "imBalances[msg.sender]: post <= pre");
  ...
  function test_put_MStableYieldSource_redeemToken_path2p1(address p_msg_sender, uint256 p_msg_value, uint256 mAssetAmount) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, mAssetAmount);
  }
}
