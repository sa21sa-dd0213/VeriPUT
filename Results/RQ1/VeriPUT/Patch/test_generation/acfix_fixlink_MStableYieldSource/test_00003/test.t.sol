...
contract MStableYieldSourceCovTest_MStableYieldSource_balanceOfToken_put2p1 is Test {
  ...
  function setUp() public {
    ...
    vm.mockCall(_esbmc_ctor_state_mock_0_0, abi.encodeWithSignature("exchangeRate()"), abi.encode(uint256(0)));
    vm.etch(_esbmc_ctor_arg_mock_0_0_ret_0, hex"60006000f3");
  }
  ...
  function _veriput_parameterized(address p_msg_sender, uint256 p_msg_value, address addr) internal {
    ...
    uint256 _pre_imBalances_addr = uint256(vm.load(address(c0), keccak256(abi.encode(addr, uint256(2)))));
    
    
    vm.deal(p_msg_sender, p_msg_value);
    vm.prank(p_msg_sender);
    (bool _esbmc_value_gate_ok, ) = address(c0).call{value: p_msg_value}(abi.encodeWithSignature("balanceOfToken(address)", addr));
    
    uint256 _post_imBalances_addr = uint256(vm.load(address(c0), keccak256(abi.encode(addr, uint256(2)))));
    assertEq(_post_imBalances_addr, _pre_imBalances_addr, "imBalances[addr]: post == pre");
    assertGe(_post_imBalances_addr, _pre_imBalances_addr, "imBalances[addr]: post >= pre");
    assertLe(_post_imBalances_addr, _pre_imBalances_addr, "imBalances[addr]: post <= pre");
  ...
  function test_put_MStableYieldSource_balanceOfToken_path2p1(address p_msg_sender, uint256 p_msg_value, address addr) public {
    _veriput_parameterized(p_msg_sender, p_msg_value, addr);
  }
}
